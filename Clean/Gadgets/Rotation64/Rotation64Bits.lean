...
@[reducible] def elaborated (off : Fin 8) : ElaboratedCircuit (F p) U64 U64 where
  main := main off
  localLength _ := 16
  ...
  output_eq _ _ := by
    ...
    intro i hi
    change var ⟨_ + i * 2 + 1⟩ + var ⟨_ + (i + 1) % 8 * 2⟩ * Expression.const _ =
           var ⟨_ + i * 2 + 1⟩ + var ⟨_ + (i + 1) % 8 * 2⟩ * Expression.const _
    rfl
...
theorem soundness (offset : Fin 8) : Soundness (F p) (elaborated offset) Assumptions (Spec offset) := by
  ...
  have h_rot_vector (i : ℕ) (hi : i < 8) :
    ...
    change (env.get (i0 + i * 2 + 1) + env.get (i0 + (i + 1) % 8 * 2) * base).val < 2 ^ 8 ∧
      (env.get (i0 + i * 2 + 1) + env.get (i0 + (i + 1) % 8 * 2) * base).val =
        xs[i].val / 2 ^ o + xs[(i + 1) % 8].val % 2 ^ o * 2 ^ (8 - o)
